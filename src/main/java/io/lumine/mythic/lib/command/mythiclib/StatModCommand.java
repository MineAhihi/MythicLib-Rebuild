    ...
    public CommandTreeNode.CommandResult execute(CommandSender sender, String[] args) {
        if (args.length < 5) {
            ...
            if (target == null) {
                ...
                if (duration <= 0L) {
                    StatModifier stat = (new StatModifier(key, statName, value, type, EquipmentSlot.OTHER, ModifierSource.OTHER));
                    stat.setCustom(true);
                    stat.register(playerData);
                } else {
                    TemporaryStatModifier stat = (new TemporaryStatModifier(key, statName, value, type, EquipmentSlot.OTHER, ModifierSource.OTHER));
                    stat.setCustom(true);
