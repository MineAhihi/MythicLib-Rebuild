...
public class StatRemoveCommand extends CommandTreeNode {
    public StatRemoveCommand(CommandTreeNode parent) {
        super(parent, "statremove");
    ...
    public CommandResult execute(CommandSender sender, String[] args) {
        if (args.length < 5) {
            ...
            if (target == null) {
                ...
                if (duration <= 0L) {
                    StatModifier stat = (new StatModifier(key, statName, baseValue - value, type, EquipmentSlot.OTHER, ModifierSource.OTHER));
                    stat.setCustom(true);
                    stat.register(playerData);
                } else {
                    TemporaryStatModifier stat = (new TemporaryStatModifier(key, statName, baseValue - value, type, EquipmentSlot.OTHER, ModifierSource.OTHER));
                    stat.setCustom(true);
